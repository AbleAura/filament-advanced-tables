{{--
    This component is injected via a render hook on every list-records page.
    It checks whether the current Livewire page uses HasAdvancedTables before rendering.
--}}
@php
    $livewire = $__livewire ?? null;
    $usesTrait = $livewire && in_array(
        \Ableaura\FilamentAdvancedTables\Concerns\HasAdvancedTables::class,
        array_keys((new \ReflectionClass($livewire))->getTraits())
    );

    // Also check parent classes for the trait
    if (!$usesTrait && $livewire) {
        $usesTrait = collect(class_uses_recursive($livewire))->contains(
            \Ableaura\FilamentAdvancedTables\Concerns\HasAdvancedTables::class
        );
    }
@endphp
...
@if ($usesTrait)
    {{-- Favorites bar: preset views + user favorite views as clickable pills --}}
    @if ($livewire->showFavoritesBar ?? false)
        <div class="filament-advanced-tables-bar w-full border-b border-gray-200 dark:border-white/10 bg-gray-50 dark:bg-gray-900/40">
            ...
            {{-- Row 1: Preset views + saved favorites --}}
            <div class="flex flex-wrap items-center gap-1.5 px-4 py-2">
                ...
                <button
                    ...
                    @class([
                        ...
                        'bg-primary-600 text-white shadow-sm' => !($livewire->activePresetViewKey) && !($livewire->activeUserViewId),
                        'bg-white dark:bg-gray-800 text-gray-600 dark:text-gray-300 border border-gray-300 dark:border-white/10 hover:bg-gray-100 dark:hover:bg-gray-700' => ($livewire->activePresetViewKey) || ($livewire->activeUserViewId),
                    ])
                >
                ...
                @foreach ($livewire->getPresetViewsCollection() as $preset)
                    <button
                        ...
                        @class([
                            'inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-medium transition-colors',
                            'bg-primary-600 text-white shadow-sm' => $livewire->activePresetViewKey === $preset->key,
                            'bg-white dark:bg-gray-800 text-gray-600 dark:text-gray-300 border border-gray-300 dark:border-white/10 hover:bg-gray-100 dark:hover:bg-gray-700' => $livewire->activePresetViewKey !== $preset->key,
                        ])
                    >
                        @if ($preset->icon)
                        ...
                        {{ $preset->label }}
                        @if ($preset->badge)
                            <span class="ml-0.5 px-1.5 py-0.5 rounded text-[10px] font-semibold
                                {{ $livewire->activePresetViewKey === $preset->key ? 'bg-white/20 text-white' : 'bg-gray-200 dark:bg-gray-700 text-gray-600 dark:text-gray-300' }}">
                                {{ $preset->badge }}
                            </span>
                        @endif
                    </button>
                ...
                {{-- Divider if both preset and user views exist --}}
                ...
                @foreach ($livewire->getFavoriteViews() as $view)
                    <button
                        ...
                        @class([
                            ...
                            'bg-white dark:bg-gray-800 text-gray-600 dark:text-gray-300 border border-gray-300 dark:border-white/10 hover:bg-gray-100 dark:hover:bg-gray-700' => $livewire->activeUserViewId !== $view->id,
                        ])
                    >
                        @if ($view->icon)
                ...
                {{-- Quick Filters --}}
                @if (!empty($livewire->getQuickFilters()))
                    <div class="flex flex-wrap items-center gap-1">
                        @foreach ($livewire->getQuickFilters() as $index => $qf)
                            @php $label = $qf instanceof \Ableaura\FilamentAdvancedTables\Support\QuickFilter ? $qf->label : ($qf['label'] ?? ''); @endphp
                            @php $icon  = $qf instanceof \Ableaura\FilamentAdvancedTables\Support\QuickFilter ? $qf->icon  : ($qf['icon']  ?? null); @endphp
                            <button
                ...
                {{-- View Manager button --}}
                <button
                    wire:click="$dispatch('open-modal', { id: 'advanced-tables-view-manager' })"
                    type="button"
                    class="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-lg text-xs font-medium bg-white dark:bg-gray-800 border border-gray-300 dark:border-white/10 text-gray-600 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors"
                    title="Manage views"
                >
        ...
        {{-- Quick Save Modal --}}
        <x-filament::modal id="advanced-tables-quick-save" wire:model.live="showSaveViewModal" :heading="__('Save View')" width="sm">
            <div class="space-y-3">
                <div>
        ...
        {{-- View Manager Modal --}}
        <x-filament::modal id="advanced-tables-view-manager" :heading="__('My Views')" width="xl">
                    ...
                    @foreach ($userViews as $view)
                        <li class="group flex items-center justify-between gap-3 px-4 py-2.5 bg-white dark:bg-gray-800 hover:bg-gray-50 dark:hover:bg-gray-700/50 transition">
                            ...
                            <div class="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition flex-shrink-0">
                                {{-- Apply --}}
                                <button wire:click="applyUserView({{ $view->id }})" type="button" title="Apply" class="p-1 rounded hover:bg-primary-50 dark:hover:bg-primary-900/20 text-gray-400 hover:text-primary-600">
                                    <x-heroicon-o-arrow-right-circle class="w-4 h-4" />
                                </button>
                                {{-- Favorite --}}
                                <button wire:click="toggleFavoriteView({{ $view->id }})" type="button" title="{{ $view->is_favorite ? 'Remove from favorites' : 'Add to favorites' }}" class="p-1 rounded hover:bg-amber-50 dark:hover:bg-amber-900/20">
                                    @if ($view->is_favorite)
                                ...
                                {{-- Set Default --}}
                                <button wire:click="setDefaultView({{ $view->id }})" type="button" title="Set as default" class="p-1 rounded hover:bg-gray-100 dark:hover:bg-gray-700">
                                    <x-heroicon-o-home class="w-4 h-4 {{ $view->is_default ? 'text-primary-500' : 'text-gray-400' }}" />
                                </button>
                                {{-- Delete --}}
                                <button wire:click="deleteUserView({{ $view->id }})" wire:confirm="Delete this view?" type="button" title="Delete" class="p-1 rounded hover:bg-danger-50 dark:hover:bg-danger-900/20 text-gray-400 hover:text-danger-500">
